...
Query.prototype.matchFilter = function(tag,field,values){
   var self = this;
   var tag = tag ? '{!tag=_' + field + '}' : '';
   var fq = 'fq=' + tag + '(';
   var options = [];

   var parser = function(v) {
      if (v[0] !== '[') { return encodeURIComponent(v); }
      /* Dumb Parsing for dates */
      var date1 = v.split(' ')[0];
      date1 = date1.split('[')[1];
      date1 = date1.split(']')[0];
      ...
      var date2 =  v.split(' ')[2];
      date2 = date2.split(']')[0];
      if (date2[0] === '[') { date2 = date2.split('[')[1]; };
      if (date2[0] !== '*') {
          date2 = new Date (date2);
          date2 = date2.toISOString();
      }
      var date = encodeURIComponent(date1 + ' ' + v.split(' ')[1] + ' ' + date2);
      date = "[" + date + "]";
      return date
   };

   var value = parser(values.pop());

   var op = field + '%3A' + '"' + value + '"';
   if (value === 'null') {
      var op = "(*:*%20-" + field + ":[*%20TO%20*])"
   }
   if (value === 'new') {
      var now = new Date();
      var d = new Date();
      d.setDate(d.getDate() - 30);
      var op = "startDate-sr%3A[" + d.toISOString() + "%20TO%20*]"
   }
   options.push(op);

   for (v in values) {
      var op = field + ":" + '"' + encodeURIComponent(format.dateISOify(values[v])) + '"';
      if (values[v] === 'null')
        op = "(*:*%20-" + field + ":[*%20TO%20*])";
      options.push(op);
   }
...
Query.prototype.facet = function(options){
   ...
   if(options.field){
      if (typeof options.field === "object"){
         for (f in options.field){
            this.parameters.push('facet.field=' + '{!ex=_' + options.field[f] + "}" + options.field[f])
         }
      } else {
